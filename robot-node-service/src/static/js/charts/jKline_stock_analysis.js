...
var analysis_kline = {
   ...
   * @param source 来源
   */
  openTitle: function (obj, index, sn, source,url) {
    var thisObj = $(obj);
    ...
    if (thisObj.parents("li").hasClass("on") > 0) {
      ...
      if(url){
        if(index == 0){
          ...
          baiduTrackEvent('wr指标','click',source);
        }
      }
    }
  ...
  initKData(data, type, sn, lastPrice, appKey) {
    if (type == 'day') {
...
//MACD KD RSI 话术
    jQuery.ajax(
        {
          ...
          success: function(rs)
          {
            ...
            // MACD 话术
            if(rs){
              if(rs.data.detail.TXTMACD.memo.length>0){
                if(type == 'day'){
                  $('#macdHuashu_day'+sn).html(rs.data.detail.TXTMACD.memo);
                }else if(type == 'week'){
                  $('#macdHuashu_week'+sn).html(rs.memo);
                }
              }else{
            ...
            //KD 话术
            if(rs){
              if(rs.data.detail.TXTKD.memo.length>0){
                if(type == 'day'){
                  $('#kdHuashu_day'+sn).html(rs.data.detail.TXTKD.memo);
                }else if(type == 'week'){
                  $('#kdHuashu_week'+sn).html(rs.memo);
                }
              }else{
            ...
            //RSI 话术
            if(rs){
              if(rs.data.detail.TXTRSI.memo.length>0){
                if(type == 'day'){
                  $('#rsiHuashu_day'+sn).html(rs.data.detail.TXTRSI.memo);
                }else if(type == 'week'){
                  $('#rsiHuashu_week'+sn).html(rs.memo);
                }
              }else{
  ...
  getFloadTxt: function (_buyData, _sellData, title) {
    ...
    if (_buyData[_buyData.length - 1][0] > _sellData[_sellData.length - 1][0]) {//绿色向下
      txt = '<span><b>' + timeUtil.getTimeStr_m_d(_buyData[_buyData.length - 1][0]) + title + '金叉</b></span>';
    } else {
      txt = '<span><b>' + timeUtil.getTimeStr_m_d(_buyData[_buyData.length - 1][0]) + title + '死叉</b></span>';
    }
    return txt;
  },
  /**
  ...
  //技术分析条件选股固定问答
  getFix: function (question,sn,source,isPopup,url) {
    sendQuestion(question, null, undefined, false);
    if(isPopup){
      analysis_kline.closePopup();
    }
    jQuery.ajax({
      type: "get",
    ...
    if(url){
      baiduTrackPageview(appKey+'/module/问答/'+ question + '/' + source);
      baiduTrackEvent(question,'click',source);
    }
            ...
            if (this.x == ohlc[i][0]) {
              if (ohlc[i][15] == '1' || ohlc[i][16] == '1' || ohlc[i][17] == '1') {
                ...
                if (ohlc[i][17] == '1') {//波段买点  短线决策
                  if (ohlc[i][5] == 1) {
                    str = timeUtil.getTimeStr_m_d(ohlc[i][0]) + '上升趋势中<br>出现短线回调低点';
                  } else if (ohlc[i][7] == 1) {
                    str = timeUtil.getTimeStr_m_d(ohlc[i][0]) + '震荡趋势中<br>出现短线回调低点';
                  } else {
                    $('#container_k_float_' + tit + sn).css("display", "none");
                  }
                } else if (ohlc[i][15] == '1') {//顶背离
                  str = '在' + timeUtil.getTimeStr_m_d(ohlc[i][0]) + '的高点比上一个高点高但对应的<br>macd值没有同步新高,后期下跌概率加大.';
                } else if (ohlc[i][16] == '1') {//底背离
                  str = '在' + timeUtil.getTimeStr_m_d(ohlc[i][0]) + '的低点比上一个低点低但对应的<br>macd值没有同步新低,后期上涨概率加大.';
              ...
              $('#chart_legend_' + tit + '_top' + sn).html('<span>' + timeUtil.getTimeStr(ohlc[i][0]) + '</span><span>' + price + '</span>');
        ...
        formatter: function () {
          ...
          for (var i = 0; i < ohlc.length; i++) {
            if (this.x == ohlc[i][0]) {
              ...
              $('#chart_legend_c_' + tit + '_top' + sn).html('<span>' + timeUtil.getTimeStr(ohlc[i][0]) + '</span><span>' + price + '</span>');
              //获取K线图表的标题
              analysis_kline.getKDayTitle(tit, sn, ma5, ma10, ma20, ma60, zsyjx);
      ...
      tooltip: {
        ...
        formatter: function () {
          ...
          if (divID.indexOf("macd") > -1) {
            ...
            analysis_kline.getMacdTitle(tit, sn, dif, dea, macd, timeUtil.getTimeStr(this.x));
          }
          if (divID.indexOf("kd") > -1) {
            ...
            analysis_kline.getKdTitle(tit, sn, k, d, timeUtil.getTimeStr(this.x));
          }
          if (divID.indexOf("rsi") > -1) {
            ...
            analysis_kline.getRsiTitle(tit, sn, rsi6, rsi12, timeUtil.getTimeStr(this.x));
          }
          if (divID.indexOf("wr") > -1) {
            ...
            analysis_kline.getWrTitle(tit, sn, wr6, wr10, timeUtil.getTimeStr(this.x));
          }
          if (divID.indexOf("cci") > -1) {
            ...
            analysis_kline.getCCiTitle(tit, sn, cci, ci, timeUtil.getTimeStr(this.x));
          }
          ...
          for (var i = 0; i < data2.length; i++) {
            if (this.x == data2[i][0]) {
              ...
              txt = analysis_kline.getChartFloat(timeUtil.getTimeStr_m_d(data2[i][0]) + title + temp);
