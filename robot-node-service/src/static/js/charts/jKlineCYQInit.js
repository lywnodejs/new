...
var newKlineCYQ = {
    ...
    getTarget_CYQ_FS: function(sn) {
        ...
        var temp =
            '<h6 class="date" id="lastShowDate'+sn+'">最后日期 '+timeUtil.getTimeStr(t)+'</h6>'+
            '<div class="box_show">'+
...
var newKlineCYQUtil={
    ...
    getHeaderHtml:function (time,open,close,high,low,zf,change,volume,amount,turnover,ma5,ma10,ma20) {
        ...
        if(turnover== -1){
            tagHeader += '<div>'+timeUtil.getTimeStr_more(time)+'</div>';
        }else{
            tagHeader += '<div>'+timeUtil.getTimeStr(time)+'</div>';
        }
