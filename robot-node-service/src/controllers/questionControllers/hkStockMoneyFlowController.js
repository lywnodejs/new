import {quotaService} from "../../service";
import commonUtil from "../../questionView/utils/commonUtil";
import timeUtil from "../../questionView/utils/timeUtil";
import config from "../../config";

let ejs = require("ejs");

// 港股通资金流向 赵波 2019.10.22
module.exports = {
  async renderTemp(req, res, next, result, params) {
    let frameId = req.query.frameId;
    let sn = new Date().getTime();

    // 取股票行情
    let tMarHkshscTop10Stocks = result.data.tMarHkshscTop10Stocks;
    let symbol = tMarHkshscTop10Stocks.marType + tMarHkshscTop10Stocks.secCode;
    let quotaResult = await quotaService.getSymbolPrice({symbol: symbol});

    let needJson = req.query.d === "j";
    let renderData = {
      sn: sn,
      info: result,
      quotaResult: quotaResult,
      params: params,
      preAnswer: result.preAnswerContent,
      noSource: req.query.noSource,
      commonUtil: commonUtil,
      timeUtil: timeUtil,
      frameId: frameId,
      host: config[process.env.NODE_ENV].host,
      config: config[process.env.NODE_ENV].resource
    };

    ejs.renderFile("./src/questionView/hkStockMoneyFlow.ejs", renderData, function (err, str) {
      // console.log(err)
      if (needJson) {
        res.send({
          info: result,
          content: str
        })
      } else {
        res.set('Content-Type', 'text/html');
        res.send(str);
      }
    });
  }

}
