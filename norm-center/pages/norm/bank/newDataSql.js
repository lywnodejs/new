...
import TableTree from '../base/add/step2/tableTree'
...
import GroupConfig from '../base/add/step2/groupConfig'
...
export default function DataSql(props) {
  ...
  const requestData = async (type) => {
    try {
      ...
      let requestTypeArr = SQL_TYPES.filter(
        (v) => v.name === editValues.columnType,
      )

      let object = {
        scriptData: codeMirror.getValue(),
        requestType:
          (requestTypeArr[0] && requestTypeArr[0].columnType) ||
          '' ||
          editValues.columnType,
        datasourceId: props.selectedItem.datasourceId,
        // props.editItem === null
        //   ? props.selectedItem.datasourceId
        //   : props.editItem.groupId,
      }

      console.log(object)
      // console.log(editValues)
      // console.log(props.editItem)
      // console.log(SQL_TYPES.filter((v) => v.name === editValues.columnType))
  ...
  const saveEdit = async (i) => {
    try {
      ...
      console.log(values)
      const datasourceId = props.selectedItem.datasourceId
      const datasourceType = props.selectedItem.dataSourceType
      const groupId = groups.find((v) => v.groupName == values.groupName)
        .groupId
      // let item = list[i]
      let columnType = SQL_TYPES.filter(
        (v) =>
          v.name === values.columnType || v.columnType === values.columnType,
      )
      columnType = (columnType[0] && columnType[0].columnType) || ''
      let obj = {
        type: 'base',
        datasourceType,
        datasourceId,
              ...
              <Row gutter="20" style={{height: '100%'}} wrap={false}>
                <Col flex="4">
                  <Row style={{flexDirection: 'column'}}>
                    <Col flex="4">
                      <p>SQL输入框：</p>
                      <div style={{height: '484px', maxWidth: '876px'}}>
                        <CodeMirror
                          value={'init'}
                          options={{
                            theme: 'monokai',
