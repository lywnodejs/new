...
const breadcrumbs = [{text: '运营配置'}, {text: '产品管理'}]
...
function body(props) {
  ...
  const jump2attr = (type, id) => {
    if (type === 'add') {
      Router.push(`/operation/product/attrs-add?productId=${id}`)
    } else {
      Router.push(`/operation/product/attrs-list?productId=${id}`)
    }
  }
  ...
  const jump2edit = (id) => {
    let url = '/operation/product/edit'
    if (id) {
      url += `?id=${id}`
    }
    Router.push(url)
