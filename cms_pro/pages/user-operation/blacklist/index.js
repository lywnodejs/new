...
const breadcrumbs = [{text: '用户运营'}, {text: '黑灰名单列表'}]
...
const Search = (props) => {
  ...
  return (
    ...
    >
      ...
      <Form.Item label="名单类型" name="crowdType">
        <Select style={{width: 120}}>
          <Select.Option value={3}>全部</Select.Option>
          <Select.Option value={3}>灰名单</Select.Option>
          <Select.Option value={4}>黑名单</Select.Option>
        </Select>
      </Form.Item>
...
function body(props) {
  ...
  const jump2export = (id) => {
    let url = '/user-operation/blacklist/export'
    if (id) {
      url += '?id=' + id
    }
    Router.push(url)
...
body.getInitialProps = async () => {
  let data = await getData({...pageParams, crowdType: 3})
