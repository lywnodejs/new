...
var analysis_kline = {
   ...
   * @param t 时间
   */
  getKDayTitle: function (type, sn, ma5, ma10, ma20, ma60, zsyjx,quota) {
    if(zsyjx == undefined){
      zsyjx = "";
    }
    var $k = '', $zsyjx = '';
    ...
    var txt = '', txt2;
    txt = '<h6><span>'+chartTimeUtil.getTimeStr(quota.time*1000)+'</span><span class="rxh_color">'+quota.newPrice+'</span></h6>'+
    '<h6><span>MA5:</span><em><span>'+ma5+'</span><span>10:'+ma10+'</span><span>20:'+ma20+'</span><span>60:'+ma60+'</span></em></h6>'
  ...
  getKDayVolTitle: function (type, sn, column, vma5, vma10) {
    ...
    var txt = '';
    txt = '<span style=" color: #C457D6; margin: 0;">' + column + '</span><span style=" color: #DE9246; margin: 0 0 0 0.625rem;">MA5:' + vma5 + '</span><span style=" color: #3E85FF; margin: 0 0 0 0.625rem;">MA10:' + vma10 + '</span>'
    $k_vol.html(txt);
    $k_vol_c.html(txt);
  },
  ...
  initMacd: function(data, type, sn, quota, appKey) {
  ...
  initKdj: function(data, type, sn, quota, appKey) {
  ...
  initRsi: function(data, type, sn, quota, appKey) {
    // RSI 话术
...
//RSI有数据  用兆军接口
  ...
  initKData: function(data, type, sn, quota, appKey) {
    var lastPrice = quota.newPrice;
    if (type == 'day') {
      ...
      if (data.DAY_EQUSHI.signals.length > 0) {
        ...
        if (type == 'day') {
          ...
          //获取K线图表的标题
          analysis_kline.getKDayTitle('day', sn, ma5[ma5.length - 1][1], ma10[ma10.length - 1][1], ma20[ma20.length - 1][1], ma60[ma60.length - 1][1], zsyjx[zsyjx.length - 1][1],quota);
          ...
          analysis_kline.createKLineChart('container_k' + sn, ohlc, column, ma5, ma10, ma20, ma60, sn, '日K', peak, trough, up, down, vma5, vma10, macdblding, macdbldi, bdmd, zsylv_total, quota);
   ...
   */
  createKLineChart: function (divID, ohlc, column, ma5, ma10, ma20, ma60, sn, type, peak, trough, up, down, vma5, vma10, macdblding, macdbldi, bdmd, zsylv_total, quota) {
          ...
          for (var i = 0; i < ohlc.length; i++) {
            if (this.x == ohlc[i][0]) {
              if (ohlc[i][15] == '1' || ohlc[i][16] == '1' || ohlc[i][17] == '1') {
                  ...
                  $('#container_k_float_' + tit +'_div'+ sn).css("display", "block");
                ...
                if(str.length>0){
                    $('#container_k_float_' + tit + sn).html( str );
                }else{
                    $('#container_k_float_' + tit +'_div'+ sn).css("display", "none");
                }
              ...
              //获取K线图表的标题
              analysis_kline.getKDayTitle(tit, sn, ma5, ma10, ma20, ma60, zsyjx, quota);
