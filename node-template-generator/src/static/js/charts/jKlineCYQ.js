...
var newKlineCYQChart={
                ...
                success: function(rs)
                {
                    if(rs){
                        ...
                        if(rs.ks){
                             ...
                             */
                            var stockList = rs.ks,
                                ma5Data =  newKlineCYQChart.calculateMA(5,stockList),
                            ...
                            if(stockList.length>0){
                                ...
                                for(var j = 0;j<len;j++){
                                    //K线
                                    ohlc.push([
                                        chartTimeUtil.dataFormatter(stockList[j].date),
                                        stockList[j].open,
                                        stockList[j].high,
                                        stockList[j].low,
                                        stockList[j].close,
                            ...
                            $('#lastShowDate'+mathRandom).html('最后日期：'+chartTimeUtil.getTimeStr(ohlc[ohlc.length-1][0]));
                ...
                success: function(rs)
                {
                    if(rs){
                        ...
                        if(rs.ks){
                             ...
                             */
                            var stockList = rs.ks,
                                ma5Data =  newKlineCYQChart.calculateMAInMin(5,stockList),
    ...
    calculateMA:function (dayCount, data) {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            if (i < dayCount - 1) {
                result.push([chartTimeUtil.dataFormatter(data[i].date),0]);
                continue;
            }
            ...
            result.push([chartTimeUtil.dataFormatter(data[i].date),Number((sum / dayCount).toFixed(3))]);
        }
    ...
    */
    ...
    creatCYQChartCss:function (chartDivID,ohlc,mouseDate,close,high,low,mathRandom,lineYAxisMax,lineYAxisMin) {
         ...
         * 换手率累加，至100.获取标准日
         */
        var turnoverSum  = 0,//成交量部分和（获取标准成交量对应的成交日的日期）
            markDate=0,//标准日
        ...
        var markDate_indexOf = chartUtil.indexOfArr(ohlc,markDate);
        ...
        for(var k = 0,len=data.length ;k<len;k++){
            ...
            if(Number(high)!=Number(low)){
                x_data.push(Number(low));
                for(var i =0;i<(Number(high)-Number(low))/space;i++){
                    x_data.push(Number(low)+space*i);
                }
            }else if(Number(high)==Number(low)){//例如乐视网，最高价与最低价相同情况
                x_data.push(Number(low));
            }
            if(x_data[x_data.length-1] < high){
                x_data.push(Number(high));
            }
