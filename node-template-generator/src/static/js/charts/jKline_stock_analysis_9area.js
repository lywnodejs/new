 ...
 * 添加历史建议
 * kline.getTarget   @param from from == 'fromAPP'表示原生嵌套的个股分析页面(待定)
...
var kline_9area = {
    ...
    initKData_area: function(data,sn){
        if(data){
             kline_9area.param.listStr = JSON.stringify(data);
            /**
             * K线数据组
             */
            var stockList = data.stockData;
            var ohlc = [];//蜡烛图数据
            ...
            if(data.dataType == 'DAY' || data.dataType == 'HOUR'){
                if(data.dataType == 'DAY'){
                    if(kline_9area.param.listStr.indexOf('newStockData') != -1){
                        if(data.newStockData.newPrice){
                            if(data.newStockData.newPrice !==0){
                                if(new Date(ohlc[ohlc.length-1][0]).getDate()  != new Date(data.newStockData.tradeAtLong).getDate()){
                                    if(data.newStockData.open !==0){
                                        ohlc.push([
                                            data.newStockData.tradeAtLong,
                                            data.newStockData.open,
                                            data.newStockData.high,
                                            data.newStockData.low,
                                            data.newStockData.newPrice,
                                            data.newStockData.preClose
                                        ]);
                                    }
                                }
                            }
                ...
                if(data.dataType == 'HOUR'){
                    if(kline_9area.param.listStr.indexOf('newStockData') != -1){
                        if(data.newStockData.newPrice){
                            if(data.newStockData.newPrice !==0){
                                var d = new Date(data.newStockData.tradeAtLong);
            ...
            if(kline_9area.param.listStr.indexOf('newPrice') != -1){//昨收，
                newPrice = data.newStockData.newPrice;
                newPrice = newPrice.toFixed(2);
            }
            if(kline_9area.param.listStr.indexOf('rise') != -1){//涨幅，
                zf = data.newStockData.rise;
                zf = zf.toFixed(2);
                if(zf > 0){
                    zf = '+'+zf;
                    zfColor = 't_red';
            ...
            if(kline_9area.param.listStr.indexOf('change') != -1){//涨跌，
                zd = data.newStockData.change;
                zd = zd.toFixed(2);
            }
            if(kline_9area.param.listStr.indexOf('tradeAtLong') != -1){//返回时间
                var date = new Date(parseInt(data.newStockData.tradeAtLong, 10));
            ...
            //     reportTxt = '<em>'+year+'-'+month+'-'+currentDate+'</em>开：<span>'+data.newStockData.open+'</span>收：<span>'+newPrice+'</span>幅：<span>'+zf+'</span>';
            // }else{
                txt = "<li><p>"+stockName+" ("+stockCode+") </p><h6>"+tradeAt+"</h6></li><li class='"+zfColor+"'><p>"+newPrice+"</p><h6>"+zd+"("+zf+"%)</h6></li>";
                reportTxt = '<span>'+year+'-'+month+'-'+currentDate+'</span><span>开<em>'+data.newStockData.open+'</em></span><span>收<em>'+newPrice+'</em></span><span>幅<em>'+zf+'</em></span>'
            // }
            $('#title_9area'+sn).html(txt);
            ...
            if(kline_9area.param.listStr.indexOf('Q_1') != -1){
                ...
                //操作建议
                var isPreEntrustBs = (data.preEntrustBs==0||data.preEntrustBs==1||data.preEntrustBs==2) ? true : false;
                var isEntrustBs = (data.entrustBs==0||data.entrustBs==1||data.entrustBs==2) ? true : false;
                if(isEntrustBs){
                    if(0 == data.entrustBs){
                        legend_txt += '';
                    }else{
                        legend_txt += kline_9area.getTXT(data.entrustBs)+'&nbsp;&nbsp;&nbsp;&nbsp;';
                    }
                }else{
                    legend_txt += '';
                }
            ...
            //话术
            $('#explain_9area'+sn).html('<b>技术面分析：</b>'+data.analysisText);
            ...
            if(kline_9area.param.listStr.indexOf('flowData') !== -1){
                if(data.flowData){
                    var flowList = data.flowData.sort(kline_9area.compare('entrustBsStartAt'));
            ...
            if(kline_9area.param.listStr.indexOf('moduleData') != -1){
                moduleData = data.directionData;
            }
            kline_9area.mapData(ohlc,difData,deaData,moduleData,sn);
        }
    },
     ...
     * @returns {number}
     */
    mapData:function (ohlc,difData,deaData,moduleData,sn){
        ...
        kline_9area.createChart (ohlc,difData,deaData,moduleData,sn);
    },
    // create the chart
    createChart:function (ohlc,difData,deaData,moduleData,sn){
