...
class ClientAxios {
    ...
    request(options) {
        ...
        return new Promise(function (resolve, reject) {
            axios(options).then(function (response) {
                // logger.debug("status:%s, request:%j, time:%d", response.status, options);
    ...
    postJsonType(url, form) {
        return this.postContentTypeJson(url, form).then(function (body) {
            return JSON.parse(body);
        });
    }

    postContentTypeJson(url, form) {
        var options = this.getJsonOptions(url, 'POST', form);
        options.headers = {'Content-Type': 'application/json;charset=UTF-8'};
        return this.request(options);
    };


    getJsonOptions = function (options, method, params) {
        params = params || {};
        var result = options || {};
        // 如果是get请求,为了简化参数,options可以只是个url字符串,这里统一为对象格式
        if (_.isString(options)) {
