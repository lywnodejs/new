...
import { Card, Form, Button, Input, Select, message, TreeSelect, Icon } from 'antd';
import NSTreeSelect from '../../../../components/NSTreeSelect';
// import { NsSelect } from '../../../../components/NSTree';
...
class NewPermissionGroup extends Component {
  ...
  validateForm = (isValid) => {
    ...
    validateFields((err, { name, description }) => {
      if (!err && isValid) {
        const action = {
          type: 'monitorPermissionGroup/addNewGroup',
          payload: {
            name,
            desc: description,
            group
  ...
  isValidOdinPermission = (params, index) => {
    ...
    return params && (params.namespace && typeof params.namespace.length === 'number' && params.namespace.length > 0) && params.role && params.approver;
  }
      ...
      <React.Fragment>
        ...
        >
          <div
            ...
            onClick={(e) => handleNSSelectClick(index)}
          >
            <Select
              size="small"
              mode="multiple"
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <ContentCard title={t('新增权限组')} className="monitor_permission_manage_new">
...
})(Form.create()(NewPermissionGroup));
