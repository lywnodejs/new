...
class FeatureRele extends React.PureComponent {
   ...
   */
  relevantFeature = () => {
    const { id, featureList } = this.props.featureGroup;

    this.props.dispatch({
      type: 'featureGroup/relevantFeature',
      payload: {
        featureGroupId: id,
  ...
  formatChecked = () => {
    ...
    const { features, featureGroup } = this.props;
    const checkedFeatures = match(features, featureGroup.featureList, (key, node) => key == node.id);
  ...
  render () {
    const { features, featureGroup, t } = this.props;
    const formItemLayout = null;

    return (
      <Modal
        afterClose={this.resetFilterFields}
        className="upm-feature-rele-group"
        title={t('分配功能')}
        destroyOnClose={true}
        ...
        <Row className="upm-tree-panel">
          <Col className="upm-tree-panel__left"
            ...
            <UTree
              ...
              checkedKeys={featureGroup.featureList}
          ...
          <Col className="upm-tree-panel__right"
            span={12}>
            <div className="upm-feature-rele-group__info upm-border-bottom">
...
export default connect(({ feature, featureGroup, global }) => {
  return {
