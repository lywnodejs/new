...
class DetailTree extends React.Component {
  ...
  handleSubmitTreeData = () => {
    const { username, permissionReviewId, record, dispatch, handleBtnLoadding, t } = this.props
    const { businessId, appId } = record
    const { checkedKeys} = this.state
    let selectedNodes = []

    // 将保留和删除节点也置位loadding状态
      ...
      <div className='review-user-detail-expand'>
        <Row>
          ...
          <Col span={4} style={{textAlign: "right"}}>
            {this.props.currUserInfo && this.props.currUserInfo.approveStatus != 0 ?
