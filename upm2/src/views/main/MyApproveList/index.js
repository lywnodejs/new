 ...
 * @Date: unknown
 * @LastEditors: GuoTeng
 * @LastEditTime: 2020-11-16 15:54:00
 */
...
class MyApproveList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      filterParams: {
  ...
  getReviewList = () => {
    ...
    dispatch({
      type: 'review/fetchMyApproveList',
      payload: params
    });
  };
  ...
  showDetail = record => {
    this.props.dispatch(
      routerRedux.push(
        MAIN + '/mini_approve/approve_my/approve-my-detail/' + record.id
      )
    );
  };
  ...
  render() {
    const { t, form, approveList, loadingFetchMyApproveList } = this.props;
    const { records, total } = approveList;
    ...
    return (
      <div className="my-approve-list upm-filter-content-page">
        {/* <Card title={t('审核人审核')} bordered={false} className="filter-area"> */}
        <div className="filter-area">
          <div className="filter-title">{t('审核人审核')}</div>
          <Form {...formItemLayout}>
        ...
        <div className="content-area">
          <Table
            ...
            loading={loadingFetchMyApproveList}
            className="upm-table"
            rowKey="id"
          />
        </div>
...
const mapStateToProps = state => {
  ...
  return {
    ...
    approveList: review.approveList,
    loadingFetchMyApproveList: review.loadings['fetchMyApproveList']
  };
};

export default connect(mapStateToProps)(Form.create()(MyApproveList));
