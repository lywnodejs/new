...
class ApplyForm extends React.Component {
    ...
    if (getIdByIdentifying(resourceTypeList, 'area') == resourceType) {
      return shouldItemRender ? (
        <div>
          ...
          <FormItem {...formItemLayout2} label={t('选择地区')}>
  ...
  getTableItem = config => {
    ...
    if (
      ...
      ReportTip = (
        <div style={{ color: 'rgb(255, 0, 0)' }} className="bigdata-report-tip">
      ...
      return (
        <React.Fragment>
          {resourceTypeList.length ? (
            <div id="intro-step2">
              <FormItem {...formItemLayout2} label={t('申请的权限类型')}>
  ...
  handleRecommendRole = role => {
    ...
    const { id: value, nameZh: label, isGroup, riskLevel } = role;
    ...
    const roles = _.uniqBy(
      _roles.concat({ value, label, key: value, isGroup, riskLevel }),
      'value'
    );
          ...
          <Col span={17}>
            <FormItem>
              {getFieldDecorator('username', {
                ...
                initialValue: { key: userInfo.id, label: userInfo.username }
              })(
                // <Input
            ...
            <FormItem>
              {!isMonitorSysId(appId) &&
                getFieldDecorator('isApplyForOthers', {
                  ...
                  <Checkbox onChange={this.changForOthers}>
                    {t('代他人申请')}
                  </Checkbox>
                )}
            </FormItem>
