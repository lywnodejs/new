...
class UTree extends React.Component {
  ...
  getMatchedNodes = searchValue => {
    ...
    traverse(nodes, node => {
      if (showName) {
        (node.name.indexOf(searchValue) != -1 || node.nameZh.indexOf(searchValue) != -1) && matchedNodes.push(node);
      } else {
        node.name.indexOf(searchValue) != -1 && matchedNodes.push(node);
      }
    });
  ...
  autoExpand = () => {
    this.setState({
      expandedKeys: this.expandedKeyList,
