...
define('js/duty/initCalendar',['moment'], function(moment) {
    function initCalendar($scope) {
        var now = new Date($scope.year, $scope.month, 1),
            current = moment(new Date()).format('YYYY-MM-DD'),
            firstCal = moment(now),
            lastCal = moment(now).endOf('month'),
            day = 0,
            results = [];
        
        function older(start, end) {
            if(start === end) {
                return 0;
            }
        ...
        //  添加本月的数据
        while(firstCal.isBefore(lastCal)) {
            var key = firstCal.format('YYYY-MM-DD');
            results.push({
                key,
                older : older(key, current),
                day : firstCal.format('D'),
                date : new Date(firstCal.valueOf()),
                shadow : false
            });
            firstCal.add({days : 1});
        }
        ...
        //  补齐本月上面的周
        while((day = lastCal.weekday()) !== 6) {
            lastCal.add({days : 1});
            var key = lastCal.format('YYYY-MM-DD');
            results.push({
                key,
                older : older(key, current),
                date : new Date(lastCal.valueOf()),
                day : lastCal.format('D'),
                shadow : true
            });
        }
...
define('js/duty/PaiBanCtrl',['moment', './initCalendar'], function (moment, initCalendar) {
    return function (appMod) {
        ...
        appMod.controller('PaiBanCtrl', ['$rootScope', '$scope', '$http', '$uibModal', '$httpParamSerializer',
            function ($rootScope, $scope, $http, $uibModal, $httpParamSerializer) {
                //  设置日期
                var now = new Date(),
                ...
                $scope.no_plan = 0;
                $scope.month_desc = function () {
                    return this.monthEnums[this.month];
                }
                $scope.currTask = null;
                $scope.groupId = null;
                ...
                $scope.fetchTasks = function () {
                    var values = initCalendar($scope),
                        month = $scope.month + 1;
                    $http({
                        method: 'GET',
                        url: '/duty/queryDutyPlanByDate',
                        params: {
                            year: $scope.year,
                            month,
                            group_id: $scope.groupId.group_id
                        }
                    }).success(function (resp) {
                        var rdatas = resp.data,
                            plans = {};
                        if (rdatas) {
                            //  转换数据
                            $scope.no_plan = rdatas.no_plan;
                            for (var i = 0, rdp = rdatas.plans, len = rdp.length; i < len; i++) {
                                plans[rdp[i].cycle] = rdp[i]
                            }
                            //  嵌入TASK
                            for (i = 0, len = values.length; i < len; i++) {
                                var task = plans[values[i].key];
                                if (!!task) {
                                    values[i].task = task;
                                } else {
                                    values[i].task = false;
                ...
                $scope.getGroupOptions = function() {
                    $http({
                        method: 'GET',
                        url: '/duty/queryMyGroup'
                    }).success(function (resp) {
                        $scope.groupOptions = resp.data;
                        if($scope.groupOptions.length > 0) {
                            $scope.groupId = $scope.groupOptions[0]
                        }
                        // $scope.search()
                        $scope.fetchTasks();
                    });
                ...
                /*默认调用*/
                $scope.getGroupOptions();
                //  选择排班人员
                $scope.createTask = function (td) {
                    $uibModal.open({
                        animation: true,
                        ...
                        size: 'lg',
                        backdrop: false,
                        controller: ['$scope', '$uibModalInstance', '$http', function (mdlScope, $uibModalInstance, $http) {
                            //  当前点击任务；
                            $scope.currTask = td;
                            mdlScope.name = '';
                            //  关闭窗口                            
                            mdlScope.close = function () {
                                $uibModalInstance.close();
                            }
                            //  保存数据
                            mdlScope.save = function () {
                                var user = null;
                                ...
                                this.nonChecked = !user;
                                //  提交数据
                                if (!this.nonChecked) {
                                    var key = $scope.currTask.key;
                                    $http({
                                        method: 'POST',
                                        url: '/duty/saveDutyPlan',
                            ...
                            angular.extend(mdlScope, {
                                ...
                                search: function () {
                                    $http({
                                        method: 'GET',
                                        url: '/duty/queryDutyEmpList',
                                        params: {
                                            page: mdlScope.currPage,
                                            size: mdlScope.itemsOfPage,
                                            name: mdlScope.name,
                                            group_id: $scope.groupId.group_id
                                        }
                                    }).success(function (resp) {
                                        if (resp) {
                                            mdlScope.totalItems = resp.total;
                                            mdlScope.pages = Math.ceil(mdlScope.totalItems / mdlScope.itemsOfPage);
                                            resp.data.forEach(function (item) {
                                                item.checked = false;
                                            })
                                            mdlScope.users = resp.data;
                                        }
                                    });
                                }
                            });
...
define('js/duty/ZhiBanCtrl',['moment', './initCalendar'], function (moment, initCalendar) {
    return function (appMod) {
        appMod.controller('ZhiBanCtrl', ['$rootScope', '$scope', '$http', '$uibModal', '$interval', '$httpParamSerializer',
            function ($rootScope, $scope, $http, $uibModal, $interval, $httpParamSerializer) {
                //  设置日期
                var now = new Date(),
                    year = now.getFullYear(),
                    month = now.getMonth();
                ...
                $scope.yearEnums = [2017, 2018, 2019, 2020];
                $scope.monthEnums = ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12'];
                $scope.emp_id = function () {
                    return $rootScope.empUser ? $rootScope.empUser.mail : null;
                }
                $scope.isCurrent = function (empId) {
                    return this.emp_id() === empId;
                }
                $scope.isToday = function () {
                    return moment(new Date()).format('YYYY-MM-DD') == moment($scope.currentDay).format('YYYY-MM-DD');
                }
                $scope.disableOn = function () {
                    return !this.isToday();
                }
                $scope.disableDown = function () {
                    return !(this.isToday() && $scope.todayDuty && $scope.isCurrent($scope.todayDuty.emp_email) && $scope.todayDuty.status == 1207);
                }
                $scope.todayDuty = null;
                $scope.durTimeFmt = function () {
                    if ($scope.todayDuty) {
                        var du = moment.duration($scope.todayDuty.durTime, 'ms'),
                            hours = du.get('hours') + du.get('days') * 24,
                            mins = du.get('minutes'),
                            ss = du.get('seconds');
                        return hours + '小时' + mins + '分' + ss + '秒'
                    }
                    return '';
                }
                $scope.notEmpty = function () {
                    return !!$scope.todayDuty;
                }
                ...
                angular.extend($scope, {
                    ...
                    // 新增授权的业务组 add by huangxiaomei 2018/04/17
                    groupId: null,
                    groupOptions: [],
                    ...
                    onDuty(status) {
                        $uibModal.open({
                            ...
                            backdrop: false,
                            controller: ['$scope', '$uibModalInstance', '$http', function (mdlScope, $uibModalInstance, $http) {
                                mdlScope.actionText = status == 1 ? '上班' : '下班';
                                mdlScope.errorText = '';
                                //  关闭窗口                            
                                mdlScope.close = function () {
                                    $uibModalInstance.close();
                                ...
                                mdlScope.save = function () {
                                    $http({
                                        ...
                                        mdlScope.errorText = '';
                                        if (resp.errno == 1003) {
                                            mdlScope.errorText = '值班同学未下班，执行上班操作失败';
                                            return;
                                        }
                                        if (resp.errno == 1005) {
                                            mdlScope.errorText = '当前用户还未开始值班，无法执行下班操作';
                                            return;
                                        }
                                        $scope.fetchTasks(function () {
                                            $uibModalInstance.close();
                    ...
                    search() {
                        var begin_time = !!$scope.startTime ? moment($scope.startTime).format('YYYY-MM-DD') : '',
                            end_time = !!$scope.endTime ? moment($scope.endTime).format('YYYY-MM-DD') : '';
                        if (!begin_time && end_time) {
                            this.queryHit = '请选择值班日期的开始时间';
                            return false;
                        }
                        if (!end_time && begin_time) {
                            this.queryHit = '请选择值班日期的结束时间';
                            return;
                        }
                        if ($scope.startTime > $scope.endTime) {
                            this.queryHit = '值班日期的结束时间不能小于开始时间';
                            return;
                        }
                        $scope.searching = true;
                        $http({
                            method: 'GET',
                            url: '/duty/queryMyDutyRecord',
                            params: {
                                page: $scope.currPage,
                                size: $scope.itemsOfPage,
                                begin_time,
                                end_time
                            }
                        }).success(function (resp) {
                            ...
                            $scope.pages = Math.ceil($scope.totalItems / $scope.itemsOfPage);
                            $scope.dutys = resp.data;
                        });
                    },
                    ...
                    changeTime() {
                        ...
                        if ($scope.year == year && $scope.month == month) {
                            ...
                            $scope.currentDay = new Date($scope.year, $scope.month, 1);
                        }
                        $scope.fetchTasks();
                    },

                    changeDate(time) {
                        $scope.currentDay = time.date;
                        $scope.fetchTasks();
                    },
                    ...
                    fetchTasks(callback) {
                        var values = initCalendar($scope),
                            duty_date = moment($scope.currentDay).format('YYYY-MM-DD'),
                            today = moment(new Date()).format('YYYY-MM-DD'),
                            isToday = (duty_date == today),
                            datas = [],
                            len = values.length,
                            temp = null,
                            i = 0;
                        ...
                        $http({
                            method: 'GET',
                            url: '/duty/queryDutyInfo',
                            params: {
                                duty_date,
                                group_id: $scope.groupId.group_id
                            }
                        }).success(function (resp) {
                            var rspDatas = resp.data,
                                plans = rspDatas.plans || [],
                                todayItem = null;

                            $scope.next_days = rspDatas.next_days;
                            $scope.records = rspDatas.records;
                            //  对records进行过滤
                            if (isToday) {
                                $scope.records.forEach(function (item) {
                                    if (item.status == 1207) {
                                        item.hidden = true;
                                        item.startTime = moment(item.begin_work_time).valueOf();
                                        item.durTime = moment(new Date()).valueOf() - item.startTime;
                                        todayItem = item;
                                    }
                                });
                                //  赋值，以免保留上次数据
                                $scope.todayDuty = todayItem;
                                if ($scope.todayDuty && $scope.todayDuty.durTime) {
                                    /****/
                                    $scope.timer = $interval(function () {
                                        $scope.todayDuty.durTime += 1000;
                                    }, 1000)
                                }
                            } else {
                                if ($scope.timer) {
                            ...
                            values.forEach(function (item) {
                                item.planed = plans.indexOf(item.key) > -1;
                                //  判断是否已选中
                                item.selected = (duty_date === item.key);
                            })
...
define('js/duty/index',['require','exports','module','../common/app','./PaiBanCtrl','./ZhiBanCtrl'],function(require, exports, module) {
    ...
    //  漏洞统计页面
    require('./PaiBanCtrl')(secApp);
    //  告警统计页面
    require('./ZhiBanCtrl')(secApp);
        
    //  初始化路由
    secApp.config(['$routeProvider', '$locationProvider', function($routeProvider, $locationProvider) {
         $routeProvider
            .when('/', {
              controller  : 'PaiBanCtrl as paiBan',
              templateUrl : buildUrl('paiBan.html'),
            ...
            .when('/zhiban', {
              controller  : 'ZhiBanCtrl as zhiBan',
              templateUrl : buildUrl('zhiBan.html'),
              reloadOnSearch : false
            })
            .otherwise('/');
