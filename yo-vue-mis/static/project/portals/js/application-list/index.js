 ...
 * @description
 */
define('js/application-list/index',['require','exports','module','../../common/angular-pagination/index','../../common/angular-bootstrap-datepicker-ppopup/angular-bootstrap-datepicker-ppopup','../common/utils','../common/form/slideForm','./../common/init-range-time/index','./../common/init-range-time/transfer-time','../application-detail/js/adapter-data','../../common/export-data/export-excel','../common/get-buttons-html','../common/config/index','../common/app','./../common/add-water-mark','../common/app-list-init','./../bug/page-config','./../bug/page-config'],function (require, exports, module) {
    ...
    var adapterData = require('../application-detail/js/adapter-data');
    ...
    var LEVEL_MAP = {
        '数据申请': ['公开数据(C1)', '内部数据(C2)', '秘密数据(C3)', '机密数据(C4)'],
        '漏洞申请': require('./../bug/page-config').riskLevel,
        '告警工单': require('./../bug/page-config').riskLevel,
    }
    ...
    app.controller('application-list', ['$rootScope', '$scope', '$http', '$timeout', '$filter', 'mypagination',
        function ($rootScope, $scope, $http, $timeout, $filter, mypagination) {
            var ctrl = this;
            var initData = utils.getHashData();
            ...
            ctrl.pageData = angular.copy(transferTime(initData));
            $scope.getDataUrl = '/process/getTaskList';
            $scope.type = initData.type || 2;
            $scope.formType = initData.formType || '';
            $scope.activeIndex = $scope.type | 0;

            $scope.getLevel = function (type, level) {
                return (LEVEL_MAP[type] || {})[level | 0] || '';
            }
            ...
            $timeout(function () {
                if ($scope.formType == '') {
                    $rootScope.$broadcast('header.cancelSelect');
                }
            });

            getButtonsHtml($rootScope, $scope, {
                "导出全部数据": '<a href="javascript:" ng-click="exportData()" class="p-btn i-fr">导出全部数据</a>'
            })
            ...
            $scope.getExtendData = function () {
                ...
                }, filterPickerData(ctrl.pageData))
            };

            $scope.$on('data', function (e, data) {
                angular.extend($scope, data);
            });
            $scope.users = ['liuqiqi', 'hongling', 'fanyonggang']
