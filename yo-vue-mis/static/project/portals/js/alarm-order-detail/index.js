 ...
 * @author xiongjian
 * @email xiongjian@didichuxing.com
 * @create date 2017-08-15 05:37:47
 * @modify date 2017-08-21 11:38:35
 * @desc [description]
 */
define('js/alarm-order-detail/index',['require','exports','module','../../common/autocomplete-muti/index','../../common/add-mark/index','../../common/show-large-img/index','../../../../lib/angular-dialog/index','../../common/angular-pagination/index','../../common/angular-bootstrap-datepicker-ppopup/angular-bootstrap-datepicker-ppopup','../../common/angular-bootstrap-timepicker/index','moment','../common/addTableClass','../common/utils','../common/table/index','./../../../../lib/es6-promise/es6-promise','../alarm/adapterAlarmData','./../alarm-order/getFields','./../common/datetimepicder-pop-init/index','./../common/form/check-form-noueditor','./../common/dialog/showSuccessDialog','./../alarm-order/redirectPage','../common/exportAlarmList','./../common/add-water-mark','../common/app','../common/require-common','../common/service/service'],function (require, exports, module) {
    ...
    var urlData = utils.getUrlData();
    var adapterAlarmData = require('../alarm/adapterAlarmData');
    var getFields = require('./../alarm-order/getFields')
    ...
    var redirectPage = require('./../alarm-order/redirectPage')
    var exportAlarmList = require('../common/exportAlarmList');
    var addWaterMark = require('./../common/add-water-mark')

    var app = require('../common/app');
    app.requires.push(
        'ui.myAutoComplete', 'ngDialog', 'ui.myAddMark', 'ui.bootstrap-datepickerPopup',
        'ui.showLargeImg', 'ngSanitize', 'ui.bootstrap-popover', 'ui.bootstrap-pagination', 'ui.bootstrap.module.timepicker');
    require('../common/require-common')(app, 3);
    require('../common/service/service')(app);

    app.controller('alarm-order-detail', ['$rootScope', '$scope', '$http', '$timeout', 'ngDialog', '$filter', '$element', 'mypagination',
        function ($rootScope, $scope, $http, $timeout, ngDialog, $filter, $element, mypagination) {
            var ctrl = this;
            var alarmControl = $scope.alarmControl = 0;
            var cacheExpireDate;
            ctrl.getDepartmentUrl = '/process/department';
            ctrl.getUsersUrl = '/process/getUserList';
            $scope.eventTableList = []
            $scope.hstep = 1
            $scope.mstep = 1
            addWaterMark($rootScope)
            //ctrl.pageData = ctrl.editData = ctrl.components = {};
            ctrl.componentKeys = [];
            var adapterData = function ($filter, data) {
                //to-show
                ['safetyEngineers', 'proceessers', 'followers', 'notifiers', 'aduiters'].forEach(function (key) {
            ...
            var getEventTableList = function(id) {
                $http({
                    method: 'GET',
                    ...
                    $scope.eventTableList = res.data
                })
            }

            ctrl.confirmInfo = function (myForm) {
                checkForm(ctrl, $element[0].querySelector('#js-confirmForm'), myForm, $timeout)
                    .then(function () {
                        return $http({
                            method: 'POST',
                            url: '/alarm/workOrder/complete',
                            data: angular.extend({
                                id: urlData.id
                            }, angular.copy(ctrl.editData))
                        }).success(function (res) {
                            if (String(res.errno) !== '0') {
                                return;
                            }
            ...
            ctrl.updateInfo = function (updateForm) {
                if (ctrl.editData.expireTime_h) {
                    ctrl.editData.expireTime = moment(ctrl.editData.expireTime).set({
                        'hour': ctrl.editData.expireTime_h ? moment(ctrl.editData.expireTime_h).hour() : 0,
                        'minute': ctrl.editData.expireTime_h ? moment(ctrl.editData.expireTime_h).minute() : 0,
                        'second': 0
                    }).toDate()
                }
                checkForm(ctrl, $element[0].querySelector('#js-updateForm'), updateForm, $timeout)
                    .then(function () {
                        return $http({
                            method: 'POST',
                            url: '/alarm/workOrder/update',
                            data: angular.extend({
                                id: urlData.id
                            }, angular.copy(ctrl.editData))
                        }).success(function (res) {
            ...
            $http({
                ...
                //set page data
                ctrl.pageData = adapterData($filter, res.data.viewInfo);
                //set editData
                ctrl.editData = res.data.editInfo;
                ctrl.editData.expireTime = moment(ctrl.editData.expireTime).toDate();
                ctrl.editData.expireTime_h = moment(ctrl.editData.expireTime).toDate();
                cacheExpireDate = angular.copy(ctrl.editData.expireTime)
                //set components
                ctrl.components = res.data.component;
                //get button key
                ctrl.componentKeys = Object.keys(ctrl.components);
                $scope.pageId = res.data.viewInfo.id;
                $scope.alarmId = res.data.viewInfo.alarmId;

                // 获取关联事件
                getEventTableList($scope.pageId)
                initTableList($scope.alarmId);
                //table预览需要加上额外的样式
                $timeout(function () {
                    //reset muti-auto input width
                    angular.element(window).triggerHandler('resize');
                    ['alarmDesc'].forEach(function (val) {
            ...
            $scope.$on('data', function (e, data) {
                ...
                angular.extend($scope, adapterAlarmData(data, {}, false));
            });
            $scope.getExtendData = function () {
