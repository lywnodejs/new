...
define('js/bug/dialog-ignore',['require','exports','module','../common/dialog/validateDialogForm','./../common/dialog/showSuccessDialog','./send-data','./../bug-detail/redirect-page'],function (require, exports, module) {
    ...
    var redirectPage = require('./../bug-detail/redirect-page');
    var cacheIgnoreReason1;
    return function (ngDialog, url, pageId, dsrcInfo) {
        var dialog = ngDialog.open({
            ...
            controller: ['$scope', '$element', '$timeout', '$http', '$rootScope', function ($innerScope, $element, $timeout, $http, $rootScope) {
                $innerScope.isRequireDesc = false;
                ...
                $innerScope.changeReason2 = function ($event) {
                    if ($innerScope.ignoreReason2 == null) {
                        return;
                    }
                    var $ignoreReason2 = $innerScope.ignoreReason2 | 0;
                    //如果选中其他下面的说明就是必须填写
                    $innerScope.isRequireDesc = $innerScope.reason2.some(function (val) {
                        return val.id === $ignoreReason2 && val.dName === '其他';
                    });
                };

                $innerScope.ignoreReason1Change = function ($event) {
                    $innerScope.ignoreReason2 = undefined;
                    if (!$innerScope.ignoreReason1) {
                        return;
                    }
                    $http({
                        method: "GET",
                        url: '/dictionary/listByParentId/' + $innerScope.ignoreReason1
                    }).success(function (res) {
                        if (res.errno == 0) {
                            $innerScope.reason2 = res.data;
                        }
                    });
                };

                if (!cacheIgnoreReason1) {
                    $http({
                        method: "GET",
                        url: '/dictionary/listByParentId/1070'
                    }).success(function (res) {
                        if (res.errno == 0) {
                            $innerScope.reason1 = cacheIgnoreReason1 = res.data;
                        }
                    });
                } else {
                    $innerScope.reason1 = cacheIgnoreReason1;
                }

                $innerScope.pageAgree = function () {
                    validateDialogForm($innerScope.ignoreForm, dialog, $innerScope, $timeout, false).then(function () {
                        sendData($http, url, {
                            holeId: dialog.pageId,
                            reason1: $innerScope.ignoreReason1,
                            reason2: $innerScope.ignoreReason2,
                            desc: $innerScope.desc,
                            effectBiz: dsrcInfo.effectBiz,
                            recurrentLevel: dsrcInfo.recurrentLevel,
                            usedLevel: dsrcInfo.usedLevel,
                            volumeLevel: dsrcInfo.volumeLevel,
                            discoverLevel: dsrcInfo.discoverLevel,
                            selfLevel: dsrcInfo.selfLevel,
                            dsrcRemark: dsrcInfo.dsrcRemark
...
define('js/bug/dialog-transfer',['require','exports','module','../common/dialog/validateDialogForm','./../common/dialog/showSuccessDialog','./send-data','./../bug-detail/redirect-page'],function (require, exports, module) {
    var validateDialogForm = require('../common/dialog/validateDialogForm');
    var showSuccessDialog = require('./../common/dialog/showSuccessDialog');;
    var sendData = require('./send-data');
    var redirectPage = require('./../bug-detail/redirect-page');
    return function (ngDialog, url, pageId) {
        var dialog = ngDialog.open({
            template: '\n                <form name="ignoreForm" class="p-form" ng-submit="pageAgree()">\n                    <div class="p-form-item">\n                        <label class="p-12-percent"><i class="i-must ">*</i>\u8F6C\u79FB\u7ED9\uFF1A</label>\n                        <div class="wrap-input p-col-10" ng-class="{\'p-error\':!followers.length&& showErrors}">\n                            <ui-my-auto-complete url="userList" placeholder="\u8BF7\u8F93\u5165\u5173\u6CE8\u4EBA" ng-model="followers" ng-cloak>\n                                <ui-select-match-item>{{$item.department.split(\'>\')[0]}}-{{$item.name}}</ui-select-match-item>\n                                <ui-auto-complete-list>{{item.department}}-{{item.name}}-{{item.email}}</ui-auto-complete-list>\n                            </ui-my-auto-complete>\n                            <span class="p-error-msg" ng-class="{\'ng-invalid\':!followers.length && showErrors}">\u8BF7\u9009\u8F6C\u79FB\u4EBA</span>\n                        </div>\n                    </div>\n                    <div class="p-form-item">\n                        <div class="wrap-input p-col-12">\n                            <textarea class="p-form-control p-reason" name="desc" ng-model="desc" maxlength="2000" placeholder="\u8BF4\u660E"></textarea>\n                        </div>\n                    </div>\n                     <div class="wrap-center-btn">\n                        <button class="p-btn" type="submit">\u786E\u8BA4</button>\n                        <button class="p-btn p-btn-gray" ng-click="dialogClose($event)">\u53D6\u6D88</button>\n                    </div>\n                </form>\n            ',
            className: 'ngdialog-theme-default p-dialog',
            plain: true,
            width: 680,
            height: 378,
            ...
            controller: ['$scope', '$element', '$timeout', '$http', '$rootScope', function ($innerScope, $element, $timeout, $http, $rootScope) {
                ...
                $innerScope.userList = '/process/getUserList';
                $innerScope.followers = [];
                $innerScope.showErrors = false;

                $innerScope.pageAgree = function () {
                    validateDialogForm($innerScope.ignoreForm, dialog, $innerScope, $timeout).then(function () {
                        sendData($http, url, {
                            holeId: dialog.pageId,
                            transferPeople: $innerScope.followers,
                            reason: $innerScope.desc
                        }, dialog).then(function (res) {
                            showSuccessDialog(ngDialog, $rootScope, {
                                msg: '转移成功',
                                successFn: function successFn() {
...
//# sourceMappingURL=dialog-transfer.js.map
;
define('js/bug/setDefaultExpireTime',['require','exports','module'],function (require, exports, module) {
    return function (ctrl) {
        if (!ctrl.pageData || !ctrl.pageData.holeType1 || !ctrl.pageData.riskLevel) {
            return;
        }
        var holeTypeData = ctrl.pageConfig.holeType1.find(function (val) {
            return ctrl.pageData.holeType1 == val.id;
        }).linkContent.cycle;
        if (!holeTypeData || !holeTypeData.length) {
            return;
        }
        var hour = holeTypeData.find(function (val) {
            return val.riskLevel == ctrl.pageData.riskLevel;
        }).repairTime | 0;
        var d = new Date();
        d.setHours(d.getHours() + hour);
        ctrl.pageData.expireTime = d;
    }
});
...
/**
 * author           xj
 * @date            2016-12-14 15:23:11
 * @email           littlebearbond@qq.com
 * @description
 */
//https://dalelotts.github.io/angular-bootstrap-datetimepicker/
define('js/bug/index',['require','exports','module','../../common/autocomplete-muti/index','../../common/add-mark/index','../../../../lib/angular-dialog/index','../../common/angular-bootstrap-datepicker-ppopup/angular-bootstrap-datepicker-ppopup','../common/utils','moment','../common/editor/load-ueditor','../common/form/checkForm','./page-config','./init-page-select','./init-datetimepicker-pop','./dialog-ignore','./dialog-transfer','./setDefaultExpireTime','./../bug-detail/dialog-pop','./../bug-detail/get-followers','./../common/add-water-mark','../common/app','../common/require-common'],function (require, exports, module) {
    ...
    var initPageSelect = require('./init-page-select');
    var initDatetimepickerPop = require('./init-datetimepicker-pop');
    var dialogIgnore = require('./dialog-ignore');
    var dialogTransfer = require('./dialog-transfer');
    var setDefaultExpireTime = require('./setDefaultExpireTime');
    var dialogPop = require('./../bug-detail/dialog-pop');
    var getFollowers = require('./../bug-detail/get-followers');
    var addWaterMark = require('./../common/add-water-mark')
    var app = require('../common/app');
    app.requires.push('ngSanitize', 'ui.myAutoComplete', 'ngDialog', 'ui.myAddMark', 'ui.bootstrap-datepickerPopup', 'ui.bootstrap-popover')
    require('../common/require-common')(app, 2);
    var urlData = utils.getUrlData();
    var isAppFn = function (data, val) {
        var isApp = false;
        var len = data.length;
        while (len--) {
            if (data[len].id == val) {
                isApp = data[len].linkContent.sourceType === 'APP';
                break;
            }
        }
        return isApp;
    ...
    app.controller('bug-ctrl', ['$rootScope', '$scope', '$http', '$element', '$timeout', 'ngDialog', '$filter',
        function ($rootScope, $scope, $http, $element, $timeout, ngDialog, $filter) {
            var ctrl = this;
            //该页面是编辑状态，这个页面存在编辑状态和新增状态
            ctrl.isEdit = !!~location.href.indexOf('hole-edit.html');
            ctrl.showErrors = false;
            ctrl.isDsrc = !!urlData.isDsrc;
            ctrl.pageConfig = pageConfig;
            ctrl.getDepartmentUrl = '/process/department';
            ctrl.getUsersUrl = '/process/getUserList';
            ctrl.editors = {};
            ctrl.holeAttachments = []

            addWaterMark($rootScope)

            //初始话页面下拉框，动态获取数据，并且级联
            initPageSelect(ctrl, $http, $timeout, $rootScope);
            
            initDatetimepickerPop(ctrl);
            dialogPop(ctrl, ngDialog, urlData.id);

            //init holeEffectInfo
            ctrl.addHoleInfo = function () {
                ctrl.pageData.holeEffectInfo.push('');
            };

            ctrl.deleteHoleInfo = function () {
                ctrl.pageData.holeEffectInfo.pop();
            };

            //form  start
            var editorArr = ['holeDetail'];
            var initEditor = loadUeditor($timeout, ctrl.editors, editorArr);

            // TODO 其他类型的漏洞不需要发这个请求
            ...
            var splitScore = function (str) {
                let score = undefined
                if (str) {
                    score = str.split('(')[1].split(')')[0].split('分')[0]
                }
                return +score
            }

            ctrl.splitSelfScore = function(id, typeId) {
                let options = []
                switch(typeId) {
                    case 65:
            ...
            ctrl.dsrcTypeChange = function () {
                if (ctrl.pageData.effectBiz && ctrl.pageData.effectBiz.length > 0 && ctrl.splitSelfScore(ctrl.pageData.effectBiz, 65) == 0 ||
                    ctrl.pageData.recurrentLevel && ctrl.pageData.recurrentLevel.length > 0 && ctrl.splitSelfScore(ctrl.pageData.recurrentLevel, 66) == 0 ||
                    ctrl.pageData.usedLevel && ctrl.pageData.usedLevel.length > 0 && ctrl.splitSelfScore(ctrl.pageData.usedLevel, 67) == 0) {
                    ctrl.pageData.selfLevel = '117'
                    ctrl.pageData.riskLevel = '4'
                }
                else {
                    if (ctrl.pageData.effectBiz && ctrl.pageData.effectBiz.length > 0 &&
                        ctrl.pageData.recurrentLevel && ctrl.pageData.recurrentLevel.length > 0 &&
                        ctrl.pageData.usedLevel && ctrl.pageData.usedLevel.length > 0 &&
                        ctrl.pageData.volumeLevel && ctrl.pageData.volumeLevel.length > 0 &&
                        ctrl.pageData.discoverLevel && ctrl.pageData.discoverLevel.length > 0) {
                        let score = Math.trunc(
                            +ctrl.splitSelfScore(ctrl.pageData.effectBiz, 65) +
                            +ctrl.splitSelfScore(ctrl.pageData.recurrentLevel, 66) +
                            +ctrl.splitSelfScore(ctrl.pageData.usedLevel, 67) +
                            +ctrl.splitSelfScore(ctrl.pageData.volumeLevel, 68) +
                            +ctrl.splitSelfScore(ctrl.pageData.discoverLevel, 69)
                        )
                        switch (score) {
                            case 0:
                                ctrl.pageData.selfLevel = '117'
                                ...
                                break
                            case 1:
                                ctrl.pageData.selfLevel = '116'
                                ctrl.pageData.riskLevel = '3'
                                break
                            case 2:
                                ctrl.pageData.selfLevel = '115'
                                ctrl.pageData.riskLevel = '3'
                                break
                            case 3:
                                ctrl.pageData.selfLevel = '114'
                                ctrl.pageData.riskLevel = '2'
                                break
                            case 4:
                                ctrl.pageData.selfLevel = '113'
                                ctrl.pageData.riskLevel = '2'
                                break
                            case 5:
                                ctrl.pageData.selfLevel = '112'
                                ctrl.pageData.riskLevel = '2'
                                break
                            case 6:
                                ctrl.pageData.selfLevel = '111'
                                ctrl.pageData.riskLevel = '1'
                                break
                            case 7:
                                ctrl.pageData.selfLevel = '110'
                                ctrl.pageData.riskLevel = '1'
                                break
                            case 8:
                                ctrl.pageData.selfLevel = '109'
                                ctrl.pageData.riskLevel = '1'
                                break
                            case 9:
                                ctrl.pageData.selfLevel = '108'
                                ctrl.pageData.riskLevel = '0'
                                break
                            case 10:
                                ctrl.pageData.selfLevel = '107'
                                ctrl.pageData.riskLevel = '0'
                                break
                            default:
                                ctrl.pageData.selfLevel = '117'
                ...
                ctrl.setDefaultExpireTime()
            }

            ctrl.dsrcLevelChange = function () {
                switch (ctrl.pageData.selfLevel) {
                    case '117':
                        ctrl.pageData.riskLevel = '4'
                        break
                    case '116':
            ...
            ctrl.handleIgnore = function() {
                if (ctrl.pageData.holeDsrcInfo) {
                    if (ctrl.pageData.holeDsrcInfo.selfLevel != ctrl.pageData.selfLevel && !ctrl.pageData.dsrcRemark) {
                        alert('请填写等级不一致说明')
                    } else {
                        ctrl.showDialogIgnore()
                    }
                } else {
                    ctrl.showDialogIgnore()
                }
            ...
            var getFormData = function () {
                ...
                if (ctrl.isEdit) {
                    getFollowers(angular.copy({
                        newFollowers: pageData.followers,
                        followers: pageData.oldFollowers,
                    }), angular.extend(pageData, {
                        followers: pageData.oldFollowers
                    }));

                    ['safetyEngineer', 'followersText', 'oldFollowers'].forEach(function (key) {
                        delete pageData[key];
                    });
                }

                //delete val
                ['holeEffectInfoPlaceholder'].forEach(function (key) {
                    delete pageData[key];
                });
            ...
            ctrl.submitData = function (url) {
                checkForm(ctrl, $scope, $timeout, initEditor.validateUeditorContent, editorArr, $element).then(function () {
                    var formData = getFormData();
                    if (ctrl.pageData.buttonState === 1) {

                    } else if (ctrl.pageData.buttonState === 5) {

                    }
                    // 期望修复时间格式化，保证和后端无时区误差
                    if (formData.expireTime) {
                        formData.expireTime = moment(formData.expireTime).format('YYYY-MM-DD HH:mm:ss')
                    }
                    if (ctrl.pageData.holeDsrcInfo != null && ctrl.pageData.holeState == '待确认') {
                        if (ctrl.pageData.holeDsrcInfo.selfLevel != ctrl.pageData.selfLevel && !ctrl.pageData.dsrcRemark) {
                            ctrl.isSubmitDataIng = false
                            alert('请填写等级不一致说明')
                        } else {
                            $http({
                                method: 'POST',
                                data: formData,
                                url: url //漏洞确认 /hole/confirm
                            }).success(function (res) {
                                ctrl.isSubmitDataIng = false;
                                window.location.href = "hole-detail.html?id=" + res.data.id;
                                //do success
                            }).error(function () {
                                ctrl.isSubmitDataIng = false;
                            });
            ...
            ctrl.showDialogIgnore = function () {
                dialogIgnore(ngDialog, '/hole/safeEnginnerIgnore', urlData.id, {
                    effectBiz: ctrl.pageData.effectBiz,
                    recurrentLevel: ctrl.pageData.recurrentLevel,
                    usedLevel: ctrl.pageData.usedLevel,
                    volumeLevel: ctrl.pageData.volumeLevel,
                    discoverLevel: ctrl.pageData.discoverLevel,
                    selfLevel: ctrl.pageData.selfLevel,
                    dsrcRemark: ctrl.pageData.dsrcRemark
                });
            };

            ctrl.showDialogTransfer = function () {
                dialogTransfer(ngDialog, '/hole/transfer', urlData.id);
            };

            ctrl.holeEffectInfoFocus = function (data, index) {
                if (ctrl.pageData.holeEffectInfo[index]) {
                    return;
                }
                ctrl.pageData.holeEffectInfo[index] = (!data && !ctrl.pageConfig.holeType1.isApp) ? 'http://' : '';
            };

            //选择漏洞类型，或者选择漏洞等级都需要更新期望修复时间
            ctrl.setDefaultExpireTime = function () {
                setDefaultExpireTime(ctrl)
            };

            // $scope.$on('holeType1Change', ctrl.setDefaultExpireTime);
            // $scope.$watch('ctrl.pageData.riskLevel', ctrl.setDefaultExpireTime);
            //选择漏洞类型二级需要设置默认的风险等级
            ctrl.holeType2Change = function () {
                if (!ctrl.pageData.holeType2) {
                    return;
                }
                var holeTypeData = ctrl.pageConfig.holeType2.find(function (val) {
                    return ctrl.pageData.holeType2 == val.id;
                });
                if (holeTypeData) {
                    ctrl.pageData.riskLevel = holeTypeData.linkContent.riskLevel;
                    ctrl.setDefaultExpireTime();
                }
            }
            ...
            ctrl.upload = function () {
                var file = angular.element(document.getElementById('hole-attachment'))[0].files[0]
                ...
                $http({
                    ...
                    // do success
                    ctrl.holeAttachments.push(file.name)
                    ctrl.pageData.holeAttachments.push(res.data)
                }).error(function () {
                    // TODO handle error
                });
            }
            ...
            ctrl.deleteAttachment = function() {
                ctrl.holeAttachments = []
                ctrl.pageData.holeAttachments = []
            }

            //选择发送变化不但后面的选择框数据需要发送变化，下面的受影响的app或者url 的placeholder 以及focus的交互也要发送变化
            angular.element(document.getElementById('js-holeType')).on('change', function () {
                var val = ctrl.pageData.holeType1;
                if (!val) {
                    return;
                }
                var data = ctrl.pageConfig.holeType1;
                var isApp = isAppFn(data, val);
                ctrl.pageData.holeEffectInfoPlaceholder = isApp ? "APP ：操作系统-名称-版本，例如：IOS-com.xiaojukeji.didi-2.1,IOS-com.xiaojukeji.psgui-2.1" : "http://";
                data.isApp = isApp;
            });
            ...
            var adapterData = function (data) {
                //object -->id
                ['holeType1', 'holeType2', 'holeSource1', 'holeSource2'].forEach(function (key) {
                    data[key] = String(data[key].id);
                });

                data.oldFollowers = angular.copy(data.followers);
                //to-show
                var keyExt;
                ['safetyEngineer', 'repairPeople', 'followers'].forEach(function (key) {
                    keyExt = key + 'Text';
                    data[keyExt] = angular.copy(data[key]);
                    data[keyExt] = angular.isArray(data[keyExt]) && data[keyExt].length ? $filter('getArrEmailLink')(data[keyExt]) : '无';
                });

                data.followers = [];
                //转换成可点击的链接
                /*['safetyEngineer', 'followersText','repairPeople'].forEach(function (key) {
                    data[key] = angular.isArray(data[key]) && data[key].length ? $filter('getArrEmailLink')(data[key]) : null;
                });*/

                data.expireTime = data.expireTime ? moment(data.expireTime).toDate() : null;
                data.holeEffectInfoPlaceholder = "http://";

                // 初始化dsrc数据
                if (data.holeDsrcInfo) {
                    data.dsrcRemark = data.holeDsrcInfo.remark
                    data.selfLevel = data.holeDsrcInfo.socLevel == 0 ? '' : data.holeDsrcInfo.socLevel + ''
                    data.effectBiz = data.holeDsrcInfo.socEffectBiz == 0 ? '' : data.holeDsrcInfo.socEffectBiz + ''
                    data.recurrentLevel = data.holeDsrcInfo.socRecurrentLevel == 0 ? '' : data.holeDsrcInfo.socRecurrentLevel + ''
                    data.usedLevel = data.holeDsrcInfo.socUsedLevel == 0 ? '' : data.holeDsrcInfo.socUsedLevel + ''
                    data.volumeLevel = data.holeDsrcInfo.socVolumeLevel == 0 ? '' : data.holeDsrcInfo.socVolumeLevel + ''
                    data.discoverLevel = data.holeDsrcInfo.socDiscoverLevel == 0 ? '' : data.holeDsrcInfo.socDiscoverLevel + ''
                }
                return data;
            };
                ...
                $http({
                    ...
                    $timeout(function () {
                        ...
                        initEditor.promise.then(function () {
                            //set ueditor content
                            ctrl.editors.holeDetail.ready(function () {
                                ctrl.editors.holeDetail.setContent(ctrl.pageData.holeDetail);
                            });
                        });
                        //get holeType
 ...
 define('js/bug-edit/index',['require','exports','module','../bug/index'],function (require, exports, module) {
     require('../bug/index');
 });
