 ...
 * @description
 */
define('js/redirect-page-new/index',['require','exports','module','../common/app','../common/utils','../common/filter/application-filter'],function (require, exports, module) {
    var app = require('../common/app');
    var utils = require('../common/utils');
    var urlData = utils.getUrlData();
    require('../common/filter/application-filter')(app);
    app.controller('redirect-page', ['$rootScope', '$scope', '$http', '$timeout', '$filter',
        function ($rootScope, $scope, $http, $timeout, $filter) {
            if (!urlData.id) {
                window.location.href = "/project/portals/pages/index.html";
                return;
            }
            $http({
                method: 'GET',
                url: '/alarm/workOrder/getRouteState' + location.search,
            }).success(function (res) {
                if (String(res.errno) !== '0') {
                    window.location.href = "/project/portals/pages/noauth.html";
                    return;
                }
                window.location.href = "/project/portals/pages/" + $filter('getRedirectPage')(res.data) + ".html?id=" + urlData.id;
            });
        }
    ]);
