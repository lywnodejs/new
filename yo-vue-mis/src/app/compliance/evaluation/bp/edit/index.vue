<template>
  <div class="evaluation-activate">
    <h6 class="evaluation-activate__title">安全评估项目 - 信息编辑</h6>
        ...
        <el-col :span="20">
          <el-form-item :label="$t('compliance.evaluation.evalTarget')">
            ...
            <el-table v-loading="objLoading" :data="object" border max-height="500" style="width: 100%">
              ...
              <el-table-column :label="$t('buttons.action')" width="240">
                <template slot-scope="scope">
                  <el-button v-if="project.feedback_enabled==1851" type="primary" size="small" @click="handleFeedbackObject(scope.row, scope.$index)">{{$t('compliance.evaluation.feedback')}}</el-button>
      ...
      <el-row>
        <el-col :span="6">
          <el-form-item :label="$t('compliance.evaluation.selfEval')" prop="assessment_self_enabled">
            ...
            {{project.assessment_self_enabled==1851?'启用':'禁用'}}
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="6">
          <el-form-item :label="$t('compliance.evaluation.msgFeedback')" prop="feedback_enabled">
            ...
            {{project.feedback_enabled==1851?'启用':'禁用'}}
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
...
export default {
  ...
  methods: {
     ...
     * 对象反馈
     */
    handleFeedbackObject(object) {

      const { project_id, business_id, object_id } = object
      const source = this.source
      this.$router.push({ path: '/compliance/evaluation/user/feedback/edit', query: { project_id, business_id, object_id, source } })
    },
    ...
    */
    ...
    handleSaveContent() {
      ...
      this.$http.post(object.id != null ? 'sa/project/scope/update' : 'sa/project/scope/add', object).then(() => {
