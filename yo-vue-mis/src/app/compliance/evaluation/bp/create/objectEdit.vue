  ...
  <div class="evaluation-business-edit">
    <el-form label-suffix="：" :model="value" :rules="rules" ref="objectForm" label-width="130px">
      <el-form-item :label="$t('compliance.evaluation.businessName')" prop="business_name">
        <el-select :value="value.business_name" @change="value => handleValueChange('business_name', value)" :placeholder="$t('manage.select')" style="width: 100%;">
          <el-option v-for="item in businessList" :key="item.business_name" :label="item.business_name" :value="item.business_name">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('compliance.evaluation.evalTarget')" class="is-required" prop="object_name">
...
import employeeSearchMixin from './mixins/employee'
...
export default {
  ...
  data() {
    var validateName = (rule, value, callback) => {
      ...
      const _business = this.value.business_name

      if (_value === '') return callback('请输入对象名称')
      if (this.originalName != _value && _.findIndex(_.filter(this.objectList, ['business_name', _business]), { object_name: _value }) != -1) {
