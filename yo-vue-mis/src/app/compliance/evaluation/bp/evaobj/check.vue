<template>
  <div class="evaluation-selfeva">
    <h6 class="evaluation-selfeva__title">安全评估项目 - 整改检查</h6>
    <!-- 评估内容 -->
                ...
                <el-row type="flex" class="row-bg" justify="end">
                  ...
                  <el-col :span="4">
                    <el-form-item label="检查结果">
                      <el-select v-model="form.checkResult" clearable placeholder="检查结果">
                        <el-option v-for="o in dictCheckResult" :key="o.id" :label="o.dName" :value="o.id"></el-option>
                      </el-select>
                    </el-form-item>
                  </el-col>
      ...
      <el-row>
        <el-col :span="24">
          <sdl-table :url="url" :query-params="vulListParams" ref="selfEvaTableList" border style="width: 100%">
            ...
            <el-table-column prop="repair_user_name" width="120" label="整改操作人">
              <template slot-scope="scope">
                <user-link :email="scope.row.repair_user_email" :key="scope.row.repair_user_id">{{scope.row.repair_user_name}}&nbsp;&nbsp;</user-link>
              </template>
            </el-table-column>
            <el-table-column prop="check_ret_name" width="100" label="检查结果">
            </el-table-column>
            <el-table-column prop="assessment_user_name" width="120" label="评估操作人">
              <template slot-scope="scope">
            ...
            <el-table-column prop="action" width="160" label="操作">
              <template slot-scope="scope">
                ...
                <el-button size="mini" type="primary" @click="()=>showSelfEva(scope.row)">检查</el-button>
              </template>
            </el-table-column>
          </sdl-table>
        </el-col>
    ...
    <el-dialog title="安全合规评估 - 安全评估详细信息" width="65%" :visible.sync="dialogSelfEvaV">
      <el-form :model="vulDetail" label-suffix="：" label-width="130px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="检查结果" prop="check_ret_name" :label-width="formLabelWidth">
              <el-select clearable v-model="vulDetail.check_ret" placeholder="选择结果">
                <el-option v-for="o in dictCheckResult" :key="o.id" :label="o.dName" :value="o.id"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="检查结果" :label-width="formLabelWidth">
              <el-input type="textarea" :autosize="{minRows:4}" placeholder="请输入建议" v-model="vulDetail.check_description"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  methods: {
    //   保存评估检查项
    saveEvaItem() {
      let { id, check_ret } = this.vulDetail
      if (check_ret == '' || check_ret == null) {
        this.$message({
          message: '请选检查结果',
          type: 'warning'
        });
      ...
      let params = {}
      params['id'] = id;
      params['check_ret'] = this.vulDetail.check_ret
      params['check_description'] = this.vulDetail.check_description
      const that = this
      this.$http.post('sa/project/object/scope/check', params)
        .then(({ body }) => {
          if (body.errno == 0) {
            that.$message({
              type: 'success',
    ...
    onSubmit() {
      const id = this.id
      const that = this;
      this.$http.post('sa/project/object/check', { id })
        .then(({ body }) => {
          if (body.errno == 0) {
            that.$message({
              type: 'success',
    ...
    getDictCheckResult() {
      this.$http.get('/dictionary/listByDataAuth/1960').then(({ body }) => {
        if (body.errno == 0) {
          this.dictCheckResult = body.data
        }
      })
    },
    // 评估
    ...
    // 检查结果
    getDictCheck() {
      this.$http.get('/dictionary/listByDataAuth/1690').then(({ body }) => {
        if (body.errno == 0) {
    ...
    getVulDetail(id) {
      return this.$http.get('/sa/vulnerability/info', { params: { id, source: this.source } }).then(({ body }) => {
        if (body.errno == 0) {
          ...
          const isExist = this.empOptions.some((item) => item.user_id == data.check_user_id)
          if (!isExist && data.check_user_id != "") {
            this.empOptions.push({
              user_id: data.check_user_id,
              user_name: data.check_user_name,
              user_email: data.check_user_email
            })
          }
          return data;
        } else {
