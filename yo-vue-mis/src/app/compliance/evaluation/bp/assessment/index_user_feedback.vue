<template>
  <div class="evaluation-activate">
    <h6 class="evaluation-activate__title">安全评估项目 - 信息反馈</h6>
    <el-form :model="project" label-width="120px" :rules="rules" ref="projectForm" label-suffix="：">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item :label="$t('compliance.evaluation.creator')">
            <user-link :email="project.create_user_email">{{project.create_user_name}}&nbsp;&nbsp;</user-link>
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item :label="$t('compliance.evaluation.endTime')">{{project.close_time}}</el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24">
          ...
          <el-form-item :label="$t('compliance.evaluation.evalTarget')">
            ...
            <el-table :data="pj.object" border max-height="500" style="width: 100%">
              ...
              <el-table-column :label="$t('buttons.action')" width="110">
                <template slot-scope="scope">
                  <el-button v-if="scope.row.feedback_status==1801" type="primary" size="small" @click="showObjectFeedback(scope.row, scope.$index)">{{$t('反馈')}}</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      tableKey: new Date().getTime() + "_" + Math.random() * 10000 + 9999,
      source: 2,
      currentBusiness: {},
      currentObject: {},
      currentContent: {},
  ...
  methods: {
    ...
    showObjectCheck(row) {
      ...
      this.$router.push({
        ...
        query: { id: row.id, source: this.source }
      })
    },
    ...
    handleSaveProject() {
      ...
      this.$http.post('/sa/project/object/info/submit', { id })
        .then(({ body: data }) => {
          if (data.errno == 0) {
            this.$message({
              message: '保存成功',
  ...
  mounted() {
    console.log(this.tableKey, "XXXXX")
