<template>
  <div class="evaluation-activate">
    <h6 class="evaluation-activate__title">安全评估项目 - 评估对象自评估</h6>
          ...
          <el-form-item :label="$t('compliance.evaluation.evalTarget')">
            ...
            <el-table :data="pj.object" border max-height="500" style="width: 100%">
              ...
              <el-table-column :label="$t('buttons.action')" width="110">
                <template slot-scope="scope">
                  <el-button v-if="scope.row.assessment_self_status==1811" type="primary" size="small" @click="showObjectFeedback(scope.row, scope.$index)">{{$t('自评估')}}</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleSaveProject() {
      ...
      this.$http.post('/sa/project/assessmentSelf', { id })
        .then(({ body: data }) => {
          if (data.errno == 0) {
            this.$message({
              message: '保存成功',
