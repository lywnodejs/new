<template>
  <div class="app-table app-normal-bg">
    <el-table style="width: 100%" :class="{'table-nowrap': isNoWrap}" :data="showData" :height="height" :max-height="maxHeight" :stripe="stripe" :border="border" :fit="fit" :show-header="showHeader" :highlight-current-row="highlightCurrentRow" :current-row-key="currentRowKey" :row-class-name="rowClassName" :row-style="rowStyle" :row-key="rowKey" :empty-text="emptyText" :default-expand-all="defaultExpandAll" :expand-row-keys="expandRowKeys" :default-sort="defaultSort" :tooltip-effect="tooltipEffect" :show-summary="showSummary" :sum-text="sumText" :summary-method="summaryMethod" @select="select" @select-all="selectAll" @selection-change="selectionChange" @cell-mouse-enter="cellMouseEnter" @cell-mouse-leave="cellMouseLeave" @cell-click="cellClick" @cell-dblclick="cellDblclick" @row-click="rowClick" @row-contextmenu="rowContextmenu" @row-dblclick="rowDblclick" @header-click="headerClick" @sort-change="sortChange" @filter-change="filterChange" @current-change="currentChange" @header-dragend="headerDragend" @expand-change="expandChange" @data-load-complete="data-load-complete">
      <slot></slot>
      <template v-for="_column in columns">
        <render-column v-if="showColumns(_column)" :key="_column.prop" :type="_column.type" :index="_column.index" :column-key="_column.columnKey" :label="_column.label" :prop="_column.prop" :width="_column.width" :min-width="_column.minWidth" :fixed="_column.fixed" :render-header="_column.renderHeader" :sortable="_column.sortable" :sort-method="_column.sortMethod" :sort-by="_column.sortBy" :resizable="_column.resizable" :formatter="_column.formatter" :show-overflow-tooltip="_column.showOverflowTooltip" :align="_column.align" :header-align="_column.headerAlign" :class-name="_column.className" :label-class-name="_column.labelClassName" :selectable="_column.selectable" :reserve-selection="_column.reserveSelection" :filters="_column.filters" :filter-placement="_column.filterPlacement" :filter-multiple="_column.filterMultiple" :filter-method="_column.filterMethod" :filter-value="_column.filterValue" :scoped-slots="_column.scopedSlots">
        </render-column>
      </template>
    </el-table>

    <div class="app-table-pagination app-border" :class="paginationClass" :style="paganationStyle" ref="pagination" v-if="pagination">
      <span v-if="showCustomList" class="demonstration">
        <el-button style="float: left;" size="mini" @click="filterColumnHandler"><i class="fa fa-gear"></i></el-button>
      </span>
      <el-pagination v-if="isLoadDataComplete" @size-change="handleSizeChange" @current-change="handleCurrentChange" :small="pgSmall" :current-page.sync="currentPage" :page-size="pgSize" :page-sizes="pageSizes" :layout="layoutSlot" :total="total">
      </el-pagination>
    </div>

    <el-dialog title="自定义显示列" :visible.sync="dialogVisible" width="40%">
      <el-transfer :titles="['可选择列', '已选择列']" v-model="selectColumn" :data="columnFeilds"></el-transfer>
...
import { interceptParams } from '@/utils/tools.js'
...
export default {
  ...
  methods: {
    init() {
      ...
      // 是否排序
      if (this.isSortable) {
        this.orderParams.sortField = this.defaultSort.prop
        this.orderParams.orderBy = this.defaultSort.order == 'descending' ? 1 : 0
      }
    },
    ...
    _getData(isPage) {
      ...
      if (this.methods === 'POST') {
        this.$http.post(this.url, interceptParams(param)).then(
          ({ body: res }) => {
            let data = res.data
        ...
        this.$http.get(this.url, { params: interceptParams(param) }).then(
          ({ body: res }) => {
            let data = res.data;
            if (Array.isArray(data)) {
              this.showData = res.data
    ...
    sortChange(...args) {
      if (this.pagination) {
        if (this.isServer) {
          if (args.length != 0) {
            this.orderParams.orderBy = args[0].order == 'descending' ? 1 : 0
            this.orderParams.sortField = args[0].prop
          }
          this._getData()
        } else {
          if (args.length != 0) {
  ...
  mounted() {
    ...
    if (!this.pagination) {
      ...
      this.orderParams.sortField = query.sortField || this.orderParams.sortField
