<template>
      ...
      <el-form :inline="true" label-width="100px" label-position="top"  :model="param" ref="param"  :rules='rules'>
        ...
        <el-form-item label="1. 根据最近一次在SDL平台完成的安全评估，您有多大可能把他推荐给您的同事" prop="nps_evaluate">
        ...
        <el-form-item v-show="param.nps_evaluate < 9 && param.nps_evaluate >= 0 && param.nps_evaluate!=null" label="2. 请问您觉得安全评估在以下哪写方面作出改善，您会愿意给出更高的分数？" prop="improve_suggest">
          <el-checkbox-group style='margin-left:20px;' v-model="param.improve_suggest">
            <el-checkbox label="体验更好（如平台操作更方便）"></el-checkbox><br>
            <el-checkbox label="效率更高（如评估的更快）"></el-checkbox><br>
            <el-checkbox label="准确度更高（如检测结果误报少）"></el-checkbox>
          </el-checkbox-group>
        ...
        <el-form-item  v-show="param.nps_evaluate >= 9" :label="'2. 请问您刚才给出 ' + param.nps_evaluate + ' 分的主要原因是？'" prop="improve_suggest">
          <el-checkbox-group  style='margin-left:20px;' v-model="param.improve_suggest">
            ...
            <el-checkbox label="体验好（如平台操作很方便）"></el-checkbox><br>
            <el-checkbox label="效率高（如评估的很快）"></el-checkbox><br>
            <el-checkbox label="准确度高（如检测结果误报少）"></el-checkbox>
          </el-checkbox-group>
          <span v-show="!flag && param.improve_suggest.length==0" class="flag-info1"><i class="el-icon-warning"></i> 此项未填，请填写完整</span>
        </el-form-item>
        <el-form-item label="3. 关于安全评估，如果您还有其他意见或建议，欢迎您的反馈:" prop="description">
            <el-input
...
export default connect(() => {
  return {
    ...
    questionnaireNew: 'baseline_requirement/questionnaireNew'
})({
  props: ['dialogVisible'],
  data() {
    return {
  ...
  methods: {
      createProject() {
          ...
          this.questionnaireNew(queryParam).then(res => {
              console.log(res)
          })
