<template>
  <div id="evaMaterial">
    <div class="doradoDesign">
      <div>
        <!-- <h3>设计安全评估材料</h3> -->
        <div class="designTitle">评估材料</div>
        <app-permission>
          <el-button
            type="primary"
            icon="el-icon-edit"
          ...
          <el-button
            ...
            size="mini"
            style="float:right;width:96px;"
            v-if="currentStatus!=103 && currentStatus!=107"
            class="designFunction-btn"
            @click="editable=!editable">
            编辑
          </el-button>
      ...
      <el-form v-if="editable" :inline="true" label-position="left" label-width="120px" class="design-content" style="margin-top: 10px;"
              :model="design">
        <el-row>
          ...
          <el-col>
            <el-form-item label="技术/部署架构图">
              ...
              <div class="fileLoad" v-show="editable && design.architecture_diagram_file"><a class="uploadShow"
                                                                                            :href="design.architecture_diagram_file"
                                                                                            target="_blank">[已上传] 技术/部署架构图</a></div>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col>
            <el-form-item label="上传jar包">
              <el-upload
                ...
                v-show="editable"
                :before-upload="beforeUpload"
                :onSuccess="jarUploadSuccess"
                :on-remove="removeJarFile"
                :limit=1
                :auto-upload="true">
                <el-button class="designBtn" size="small" type="primary">选取文件</el-button>
                <div slot="tip" class="el-upload__tip">多个文件请打包上传，文件大小不超过50Mb，大文件请上传Cooper，并附云盘链接于备注信息</div>
      ...
      <div v-if='disabled=!editable' class="designContentDisplay">
        <div class="items-left">
          <div class="items">
            <span class="c1">项目Wiki地址 :</span>
            <!-- <span class="c2">{{design.wiki_link}}</span> -->
            <p class="c2" v-for="(item, index) in design.wiki_link" :key="index">
              <a class="uploadShow"  :href="item.value" target="_blank">{{ item.value }}</a>
            </p>
          </div>
          <div class="items">
            <span class="c1">接口文档地址 :</span>
            <!-- <span class="c2">{{design.interface_desc}}</span> -->
            <p class="c2" v-for="(item, index) in design.interface_desc" :key="index">
              <a class="uploadShow"  :href="item.value" target="_blank">{{ item.value }}</a>
            </p>
          </div>
          <div class="items">
            <span class="c1">备注信息 :</span>
            <span class="c2">{{design.description}}</span>
          </div>
        </div>
        <div class="items-right">
          <div class="items">
            <span class="c1">PRD设计文档 :</span>
            <div v-show="design.prd_doc_file">
              <a class="uploadShow" :href="design.prd_doc_file" target="_blank">[已上传] PRD设计文档</a>
            </div>
            <!-- <span class="c2">{{design.description}}</span> -->
          </div>
          <div class="items">
            <span class="c1">技术/部署架构图 :</span>
            <div v-show="!editable && design.architecture_diagram_file" class="">
              <a class="uploadShow" :href="design.architecture_diagram_file" target="_blank">[已上传] 技术/部署架构图</a>
            </div>
            <!-- <span class="c2">{{design.description}}</span> -->
          </div>
        </div>
      </div>
      <div class="codeEvaluationModule">
      ...
      <div v-show="editable" class="submitButton">
        <el-button class="design-button" type="warning" @click="throttleButton(submitDesignInfo)" :disabled="isDisabled">提交评估材料</el-button>
...
<script>
  ...
  export default connect(() => {
    ...
    data() {
      return {
        uploadUrl: '/api/dorado/baseline/material/uploadFile',
        url: '/api/common/file/upload',
        uploadJarUrl: '/api/common/file/upload',
        prdData: {
          type: 'prd',
    ...
    methods: {
      fetchData() {
        ...
        ajax.post(API.getBaselineDesignInfo, postJson).then(response => {
          ...
          if (errno === 0) {
            ...
            if (data) {
              ...
              if (response.data.architecture_diagram_file != '') {
                this.architecture_diagram_file_list.push({
                  name: '[已上传] 技术/部署架构图',
      ...
      submitDesignInfo() {
        if (this.myWhiteEvaluation.white_eva_list === undefined) {
          this.$message.error('提交失败，请提交代码仓库地址');
          return
        }
        ...
        ajax.post(API.submitBaselineMaterial, postJson).then(response => {
          const errno = response.errno
          const errmsg = response.errmsg
          if (errno === 0) {
            ...
            this.$alert(`请给安全工程师 <strong>${this.enginner}</strong> 开Git仓库的reporter权限、wiki权限`, '提示', {
      ...
      deleteUploadFile(postJson) {
        ajax.post(API.deleteBaselineUploadFile, postJson).then(response => {
          const errno = response.errno
          const errmsg = response.errmsg
          if (errno === 0) {
            const data = response.data
