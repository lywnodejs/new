<template>
  <div id="ocean-baseline-scanTaskTime-chart">
       
      <el-row :gutter="20">
        <el-col :span="24">
          <div class="content-chart">
            <div id="baseline-by-scanTaskTime" class="chart-content"></div>
  ...
  export default connect(() => {
    ...
    methods: {
      fetchData() {
          ...
          this.baselineScanTaskByTime(param).then(res => {
              this.scanTaskInfo = res
              this.data = [
                ['项目平均时间', parseFloat(this.handleTimeByDay(this.scanTaskInfo.avg.all)), this.handleTime(this.scanTaskInfo.avg.all)],
                ['A|B类平均时间', parseFloat(this.handleTimeByDay(this.scanTaskInfo.avg.ab)), this.handleTime(this.scanTaskInfo.avg.ab)],
                ['C|D类平均时间', parseFloat(this.handleTimeByDay(this.scanTaskInfo.avg.cd)), this.handleTime(this.scanTaskInfo.avg.cd)],
                ['最大时间', parseFloat(this.handleTimeByDay(this.scanTaskInfo.max)), this.handleTime(this.scanTaskInfo.max)],
                ['最少时间', parseFloat(this.handleTimeByDay(this.scanTaskInfo.min)), this.handleTime(this.scanTaskInfo.min)]
              ]
      ...
      drawBaselineChart(res) {
        let baselineChart = echarts.init(document.getElementById('baseline-by-scanTaskTime'))
        let option = {
          color: ['#7CE0C3', '#9DC5F3', '#73A6F7', '#2665B4'],
          title: {
            text: 'fatbird首轮扫描时长/分钟',
            left: 20,
            top: 20,
            ...
            feature: {
              dataView: {
                ...
                optionToContent: function(opt) {
                  ...
                  let str = `<div class="content-header">fatbird首轮扫描时长：</div>
...
<style lang="less">
#ocean-baseline-scanTaskTime-chart{
    .content-chart {
