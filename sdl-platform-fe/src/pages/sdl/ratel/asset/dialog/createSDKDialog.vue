<template>
       ...
       <el-form :inline="true" label-width="80px" label-position="left" >
        ...
        <el-form-item label="SDK最近提交者">
          <el-input class="inputWidth"
                    v-model="queryParam.sdk_git_last_committer"
...
export default connect(() => {
  return {
    ...
    updateAssetSDK: 'ratel_assets/updateAssetSDK',
    createAssetSDK: 'ratel_assets/createAssetSDK'
})({
  props: ['dialogVisible', 'scopeRows'],
  data() {
    return {
      title: '',
      ...
      createOrUpdate: null,
      isTrue: [{label: '是', value: 1}, {label: '否', value: 0}]
    }
  },
  created() {
  },
  ...
  methods: {
    createInfo() {
        this.createAssetSDK(this.queryParam).then(res => {
            this.createDialogVisible = false
            this.$parent.fetchData()
        })
    },
    updateInfo() {
        this.updateAssetSDK(this.queryParam).then(res => {
            this.createDialogVisible = false
            this.$parent.fetchData()
        })
    }
