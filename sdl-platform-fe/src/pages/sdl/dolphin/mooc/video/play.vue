<template>
    <div id="dolphin-mooc-detail">
        <video id="my-video" class="video-js vjs-default-skin" controls preload="auto">
            <source id="playVideos" :src='value' type="application/x-mpegURL">
        </video>
        <comments></comments>
        <!-- <evaluate-dialog :dialogVisible='evaluateDialogVisible' @evaluate='getFormVisible'></evaluate-dialog> -->
...
export default connect(() => {
  ...
  authQuestionnaire: 'dolphin_mooc/authQuestionnaire'
})({
    data() {
        return {
    ...
    components: {comments, evaluateDialog},
    methods: {
        videoPlay() {
            let id = this.$route.query['video_id']
            this.getVideoPlay({video_id: Number(id)}).then(res => {
                this.value = res
                this.playVideo(res)
                this.video(res)
            })
        ...
        video: function(res) {
            ...
            videojs('my-video', {
                ...
                this.on('ended', function() {
                    console.log('结束播放');
                    that.finishVideoMooc({video_id: Number(that.$route.query['video_id'])}).then(res => {

                        that.authQuestionnaire({video_id: Number(that.$route.query['video_id'])}).then(res => {
                            if (res) {
            ...
            //              that.oldTime = this.currentTime();
            //         }
            //     });
            //     this.on('play', function() {
            //         isMousedown = false;
            //         that.newTime = this.currentTime();
            //         if (that.newTime < that.maxTime) {
            //             this.currentTime(that.newTime);
            //         } else {
            //             this.currentTime(that.oldTime);
            //         };
            //     });

            //     // $('.vjs-progress-holder').mousedown(function() {
            //     //     isMousedown = true;
            //     //     that.oldTime = this.getCache().currentTime;
            //     // });
            //     this.on('timeupdate', function() {
            //         if (this.currentTime() > that.maxTime && !isMousedown) {
            //             that.maxTime = this.currentTime();
            //         }
            //     });
            //   }
            })
        ...
        getFormVisible(val) {
            console.log(val)
            this.evaluateDialogVisible = val
        },
        createProject() {
