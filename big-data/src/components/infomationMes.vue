<template>
    <div>
        ...
        <div class="appendBox_info_Mes appendBox_Common">
...
<script>
    ...
    export default {
        ...
        methods:{
            flite_click(){//父组件调用
                this.$refs.commonSelectPage.fliteData(this.inputVal || '',1,true)
            ...
            fliteData(data){
                if(this.isgetData){
                    ...
                    this.getData()
                    //调用子组件方法
                    this.$refs.commonSelectPage.fliteData(this.inputVal || '',1)
            ...
            async getData(flag) {
                ...
                if($.trim(info.content)){
                    ...
                    if(this.twoApiClik){
                         window.scrollTo(0,0);
                        $('.appendBox_info_Mes').html('').append(info.content);
                    }else if(this.page.cp >1){
                        $('.appendBox_info_Mes .rxhBD_main').append(info.content);
                    }else {
                        $('.appendBox_info_Mes').append(info.content);
                    }
                }else {
                    $('.appendBox_info_Mes').html('');
                    this.noData = true;
                    this.moreData = false;
                }
            },
