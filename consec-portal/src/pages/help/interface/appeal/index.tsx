...
class HelpAppeal extends React.Component<IBase, IState> {
    ...
    return (
      <div className="project-help-interface-complain">
        ...
        <p className="project-help-interface-complain__introduce">
          {t('内容申诉：用户对违规内容进行申诉。')}<br />
          {t('用户申诉：用户对违规用户进行申诉。')}
        </p>

        <p className="project-help-interface-complain__introduce--title">
          {t('用户申诉流程图')}
          <img src={require('../../../../assets/appeal.png')} alt="" />
        </p>
        <p className="project-help-interface-complain__introduce" style={{ marginBottom: '40px' }}>
          {t('1、用户发起内容申诉或用户申诉后，申诉数据通过业务方同步给内容安全部。')}<br />
          {t('2、业务方需提前处理申诉数据，并按内容安全部数据结构组装申诉数据。')}<br />
          {t('3、内容安全部会针对申诉数据进行相应的处理，需要业务方提供申诉回调接口，内容安全部人工审核后，会把处置结果通过回调接口同步回业务方')}
        </p>
...
' -H "didi-header-rid: 6446a2315976af4756f297987eaa0002"  http://10.88.128.40:8000/sec/consec/open-api/appeal/cs_cc_vtmc
###返回：
系统或格式错误：apicode!=200
