...
import SearchForm from '~/components/pages/collection/report/overdue/SearchForm'
import TableList from '~/components/pages/collection/report/overdue/TableList'

const breadcrumbs = [{text: '贷后管理'}, {text: '报表统计'}, {text: '逾期报表'}]
...
function body({collectionProducts, collectionLevels, userList}) {
  ...
  const fetchList = async (values = {}) => {
    try {
      ...
      params.beginPlanRepayTime = (time && time[0].format('YYYY-MM-DD')) || null
      params.endPlanRepayTime = (time && time[1].format('YYYY-MM-DD')) || null
      ...
      const {
        data: {data, code},
      } = await api.fetch_report_overdueOrders({
        ...pageParams,
        ...params,
