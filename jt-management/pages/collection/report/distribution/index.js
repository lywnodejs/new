...
import SearchForm from '~/components/pages/collection/report/distribution/SearchForm'
import TableList from '~/components/pages/collection/report/distribution/TableList'
...
function body({collectionProducts, collectionLevels, userList}) {
  ...
  const fetchList = async (values = {}) => {
    try {
      ...
      const {
        data: {data, code},
      } = await api.fetch_report_distribution({
        ...pageParams,
        ...params,
