...
function body(props) {
  ...
  const fetchIncometList = async (values = {}) => {
    try {
      ...
      const {
        data: {data, code},
      } = await apiAccounting.fetch_deductList_list({...pageParams, ...values})
      if (code == 0) {
  ...
  const exportExcel = async () => {
    ...
    try {
      if (Array.isArray(values.time)) {
        postData = {
          accountDayStart: moment(values.time[0]).format('YYYY-MM-DD'),
          accountDayEnd: moment(values.time[1]).format('YYYY-MM-DD'),
        }
      }
      delete values.time
      const {
        data: {data, code},
      } = await apiAccounting.fetch_deductList_export({
        ...values,
        ...postData,
      })
      if (code == 0) {
      ...
      <Space direction="vertical" size="large" style={{width: '100%'}}>
        ...
        >
          ...
          <Form.Item label="借据号" name="orderNum">
            <Input placeholder="请输入借据号" style={{width: 140}} />
          </Form.Item>
          <Form.Item label="流水号" name="bankOrderNum">
            <Input placeholder="请输入流水号" style={{width: 140}} />
          </Form.Item>
          <Form.Item label="借款人姓名" name="name">
            <Input placeholder="请输入借款人姓名" style={{width: 140}} />
          </Form.Item>
          <Form.Item label="借款人手机号" name="mobilePhone">
            <Input placeholder="请输入借款人手机号" style={{width: 200}} />
          </Form.Item>
