...
const fetchList = async (
  ...
  return await biFetch('/core/index/queryByProduct', {
    startDate,
    endDate,
    ...params,
  })
...
const columns = [
  ...
  {title: '产品', dataIndex: 'productName', width: 100},
...
const Chart = ({products, list, form}) => {
  const [value, setValue] = useState('loanOverApplyRate')

  let data = []
  let productId = form.getFieldsValue()['productId']
  ...
  temp.forEach((v) => {
    ...
    res = [...res].reverse()
    data.push({name: v.productName, data: res, color: getRandomColor()})
  })
    ...
    <div className={style.chart}>
      ...
      <div className={style.radios}>
        <Radio.Group onChange={onChange} value={value}>
          ...
          <Radio value={'m1RecoveryRate'}>入催率</Radio>
  ...
  return (
    <Layout breadcrumbs={breadcrumbs}>
      <Space direction="vertical" size="large" style={{width: '100%'}}>
        ...
        <Chart list={data} products={products} form={form} />
        <TableList list={data} />
      </Space>
    </Layout>
  )
