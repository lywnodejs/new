...
const TableList = ({
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        return findOne ? (
          record.isBlue ? (
            <span style={{color: '#1890ff'}}>{findOne.description}</span>
          ) : (
            findOne.description
          )
        ) : (
    ...
    {
      title: '初审信审员',
      dataIndex: 'firstOperatorName',
      key: 'firstOperatorName',
      width: 180,
      render: (text, record, index) => {
        return record.isBlue ? (
          <span style={{color: '#1890ff'}}>{text}</span>
  ...
  const checkDetail = (row) => {
    let url = `/credit/examine/detail?creditId=${row.creditId}&orderId=${row.orderId}&flag=3`
    Router.push(url)
  }
