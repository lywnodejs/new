function TagCould(container,options){
  this.radius = window.innerWidth/2.8;
  if(this.radius>=200){
    this.radius =200;
  }
...
TagCould.prototype.start = function(){
  ...
  tags.addEventListener('mousemove',function(evt){
    var oEvent=window.event || evt;
    self.onmove(window.event || evt);
  }, false);

  tags.addEventListener('touchstart',function(e){
    if(e.target.tagName=='IMG' || e.target.tagName=='INPUT'){
      return;
    }
    self.active = true;
  }, false);
  tags.addEventListener('touchmove',function(evt){
    if(evt.target.tagName=='IMG' || evt.target.tagName=='INPUT'){
      return;
    }
    self.onmove(window.event || evt);
  }, false);
  tags.addEventListener('touchend',function(evt){
    if(evt.target.tagName=='IMG' || evt.target.tagName=='INPUT'){
      return;
    }
    self.active = false;
  ...
  window.setarrnav = setInterval(function(){
    self.update();
  }, 10);
...
TagCould.prototype.doPosition = function(){
  ...
  for(var i=0; i < this.mcList.length; i++){
    ...
    this.aA[i].style.fontSize = Math.ceil(24 * this.mcList[i].scale / 2)  + 'px';
